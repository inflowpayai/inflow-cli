...
describe('runInspectPipeline — accepts decoding', () => {
  it.each([false, true])('distinguishes empty seller offers from filtered offers: %s', async (filtered) => {
    const document = paymentRequired();
    const header = encodePaymentRequiredHeader({
      ...document,
      accepts: filtered ? document.accepts.map((offer) => ({ ...offer, scheme: 'upto' })) : [],
    });
    mock402(header);
    const events = await collect({ url: SELLER, probeOptions: { method: 'GET', headers: {} } });
    const event = events[0];
    if (event?.type !== 'accepts') throw new Error('Expected accepts result');
    const section = buildX402Section({
      status: 402,
      headers: new Headers({ [HEADERS.PAYMENT_REQUIRED]: header }),
      bytes: new Uint8Array(),
      contentType: undefined,
    });
    if (section.kind !== 'accepts') throw new Error('Expected accepts section');
    const warning = filtered
      ? 'This endpoint only offers Permit2 or upto payments, which InFlow treasury payments cannot authorize.'
      : undefined;
    expect(event.result.warning).toBe(warning);
    expect(section.warning).toBe(warning);
    expect(event.result.accepts).toEqual([]);
    expect(section.accepts).toEqual([]);
  });
